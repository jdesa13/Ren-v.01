...
set (GENERIC_INCLUDE_PATHS
    ...
    ${OPENEXR_CUSTOM_INCLUDE_DIR}/IlmBase
    ${OPENEXR_CUSTOM_INCLUDE_DIR}/IlmBase/Iex
    ${OPENEXR_CUSTOM_INCLUDE_DIR}/IlmBase/Half
    ${OPENEXR_CUSTOM_INCLUDE_DIR}/IlmBase/Imath
    ${OPENEXR_CUSTOM_INCLUDE_DIR}/OpenEXR
    ${OPENEXR_CUSTOM_INCLUDE_DIR}/OpenEXR/IlmImf
    ...
    if(WIN32)
       set(GENERIC_INCLUDE_PATHS ${GENERIC_INCLUDE_PATHS} 
        ${OPENEXR_CUSTOM_INCLUDE_DIR}/IlmBase/config.windows 
        ${OPENEXR_CUSTOM_INCLUDE_DIR}/OpenEXR/config.windows)
    endif()
...
find_path (ILMBASE_INCLUDECONFIG_PATH   IlmBaseConfig.h
           PATHS ${GENERIC_INCLUDE_PATHS}  NO_DEFAULT_PATH)
find_path (ILMBASE_IEX_PATH  IexThrowErrnoExc.h
           PATHS ${GENERIC_INCLUDE_PATHS}  NO_DEFAULT_PATH)
find_path (ILMBASE_HALF_PATH  half.h
           PATHS ${GENERIC_INCLUDE_PATHS}  NO_DEFAULT_PATH)
find_path (ILMBASE_IMATH_PATH  ImathFun.h
           PATHS ${GENERIC_INCLUDE_PATHS}  NO_DEFAULT_PATH)
get_filename_component(ILMBASE_PATH ${ILMBASE_IEX_PATH} PATH)
SET(ILMBASE_INCLUDE_PATH ${ILMBASE_PATH} ${ILMBASE_INCLUDECONFIG_PATH} ${ILMBASE_IEX_PATH} ${ILMBASE_HALF_PATH} ${ILMBASE_IMATH_PATH} ${OPENEXR_CUSTOM_LIB_DIR})

find_path (OPENEXR_INCLUDECONFIG_PATH   OpenEXRConfig.h
           PATHS ${GENERIC_INCLUDE_PATHS}  NO_DEFAULT_PATH)
find_path (OPENEXR_IlmImf_PATH  ImfArray.h
           PATHS ${GENERIC_INCLUDE_PATHS}  NO_DEFAULT_PATH)
get_filename_component(OPENEXR_PATH ${OPENEXR_IlmImf_PATH} PATH)
SET(OPENEXR_INCLUDE_PATH ${OPENEXR_PATH} ${OPENEXR_INCLUDECONFIG_PATH} ${OPENEXR_IlmImf_PATH} ${OPENEXR_CUSTOM_LIB_DIR})
...
foreach (COMPONENT ${OPENEXR_CUSTOM_LIB_DIR})
  SET(GENERIC_LIBRARY_PATHS ${GENERIC_LIBRARY_PATHS} ${COMPONENT})
endforeach ()

set (GENERIC_LIBRARY_PATHS
    ${GENERIC_LIBRARY_PATHS}
    ${OPENEXR_CUSTOM_LIB_DIR}/Debug
    ${OPENEXR_CUSTOM_LIB_DIR}/Release
    ${OPENEXR_HOME}/lib
    ${ILMBASE_HOME}/lib
...
set (_openexr_components_release IlmThread IlmImf Imath Iex Half)
set (_openexr_components_debug IlmThreadd IlmImfd Imathd Iexd Halfd)

foreach (COMPONENT ${_openexr_components_release})
...
foreach (COMPONENT ${_openexr_components_debug})
    string (TOUPPER ${COMPONENT} UPPERCOMPONENT)
    # First try with the version embedded
    set (FULL_COMPONENT_NAME ${OPENEXR_CUSTOM_LIB_PREFIX}${COMPONENT}-${OPENEXR_VERSION_MAJOR}_${OPENEXR_VERSION_MINOR}${OPENEXR_CUSTOM_LIB_SUFFIX})
    find_library (OPENEXR_${UPPERCOMPONENT}_LIBRARY_DEBUG ${FULL_COMPONENT_NAME}
                  PATHS ${GENERIC_LIBRARY_PATHS} NO_DEFAULT_PATH)
    # Again, with no directory restrictions
    find_library (OPENEXR_${UPPERCOMPONENT}_LIBRARY_DEBUG ${FULL_COMPONENT_NAME})
    # Try again without the version
...
if (ILMBASE_INCLUDE_PATH AND OPENEXR_INCLUDE_PATH AND
    ...
    SET(OPENEXR_LIBRARY_RELEASE ${OPENEXR_ILMIMF_LIBRARY})
    SET(OPENEXR_LIBRARY_DEBUG ${OPENEXR_ILMIMFD_LIBRARY_DEBUG})
    select_library_configurations(OPENEXR)
    SET(ILMBASE_LIBRARY_RELEASE ${OPENEXR_IMATH_LIBRARY} ${OPENEXR_IEX_LIBRARY} ${OPENEXR_HALF_LIBRARY} ${OPENEXR_ILMTHREAD_LIBRARY} ${ILMBASE_PTHREADS})
    SET(ILMBASE_LIBRARY_DEBUG ${OPENEXR_IMATHD_LIBRARY_DEBUG} ${OPENEXR_IEXD_LIBRARY_DEBUG} ${OPENEXR_HALFD_LIBRARY_DEBUG} ${OPENEXR_ILMTHREADD_LIBRARY_DEBUG} ${ILMBASE_PTHREADSD_DEBUG})
    select_library_configurations(ILMBASE)

    SET(OPENEXR_LIBRARIES ${OPENEXR_LIBRARY} ${ILMBASE_LIBRARY})
endif ()
